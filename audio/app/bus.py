...
class AudioChunk:
    ...
    source_id: str
    seq: int
    meta: dict  # e.g., {"rms": float, "ts": float}
...
class Subscriber:
    def __init__(self, subscriber_id: str, queue_max: int = 128) -> None:
...
class AudioBus:
    def __init__(self) -> None:
        ...
        self._active_rms = 3e-4
        self._active_timeout = 0.500
    ...
    async def _mix(self, subset: Dict[str, AudioChunk]) -> Optional[AudioChunk]:
        ...
        if not actives:
            return None
        length = min(len(x.data) for x in actives)
        if length == 0:
            return None
        stacks = np.stack([x.data[:length] for x in actives], axis=0)
    ...
    async def _loop(self, period_ms: int) -> None:
        try:
            ...
            while self._running:
                ...
                await asyncio.sleep(max(0, (period_ms - elapsed_ms) / 1000))
