...
export const r2Adapter = {
  async getSignedUrl(key: string, expiresIn: number) {
    ...
    const canonicalRequest = [
      'GET',
      `/${key}`,
      `X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=${encodeURIComponent(credential)}&X-Amz-Date=${datetime}&X-Amz-Expires=${expiresIn}&X-Amz-SignedHeaders=${signedHeaders}`,
    ...
    const dateKey = crypto.createHmac('sha256', `AWS4${R2_SECRET}`).update(date).digest();
    ...
    return `https://${R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${R2_BUCKET}/${key}?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=${encodeURIComponent(credential)}&X-Amz-Date=${datetime}&X-Amz-Expires=${expiresIn}&X-Amz-SignedHeaders=${signedHeaders}&X-Amz-Signature=${signature}`;
  },
}; 