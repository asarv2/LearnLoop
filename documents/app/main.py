...
class CreateRequest(BaseModel):
    template_id: UUID = Field(
        description="UUID of the template module (filename without .py)"
    )
    kwargs: dict[str, Any] = Field(
        default_factory=dict, description="Arguments for the template's Args model"
    )
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            if e.response.status_code == 404:
                raise HTTPException(
                    status_code=404,
                    detail=f"Template {template_id} not found in storage.",
                )
            else:
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to download template: {e.response.text}",
                )
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            response = await client.post(
                storage_url, headers=headers, json=request_body
            )
            response.raise_for_status()
            data = response.json()
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to list templates: {e.response.text}"
            )
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Storage error: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"template_{template_id}", temp_path
            )
            if spec is None or spec.loader is None:
                raise HTTPException(
                    status_code=500, detail="Could not load template module spec."
                )
...
def _get_template_contract(
    mod: Any,
) -> tuple[type[BaseModel], Callable[[BaseModel], bytes], str | None, str | None]:
    """
    ...
    if not hasattr(mod, "render"):
        raise HTTPException(
            status_code=500, detail="Template missing render(args) function."
        )
    ArgsModel = mod.Args
    ...
    if not issubclass(ArgsModel, BaseModel):
        raise HTTPException(
            status_code=500, detail="Template Args must subclass pydantic.BaseModel."
        )
...
"""
        ...
        def _find_stray_line_continuations(src: str) -> int | None:
        ...
        def _check_latex_stray_backslashes(tex_content: str) -> str | None:
            import re
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.match(r"^[^%]*?\\\s*$", line):
                    return (
                        f"Line {i}: LaTeX line ends with lone backslash: {line.strip()}"
                    )
        ...
        try:
            ...
            render_func = temp_module.render
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if tex_files:
                            with open(
                                tex_files[0], encoding="utf-8", errors="ignore"
                            ) as tex_file:
                                tex_content = tex_file.read()
                                latex_error = _check_latex_stray_backslashes(
                                    tex_content
                                )
                                if latex_error:
                                    return JSONResponse(
                    ...
                    try:
                        ...
                        if log_files:
                            with open(
                                log_files[0], encoding="utf-8", errors="ignore"
                            ) as log_file:
...
async def get_template_spec(template_id: UUID) -> JSONResponse:
    ...
    try:
        ...
        ArgsModel, _, default_filename, template_description = _get_template_contract(
            mod
        )
...
async def create_document(req: CreateRequest) -> StreamingResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail=json.loads(str(e).replace("'", '"'))
        )  # best-effort
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{_safe_filename(filename)}.pdf"'
        },
    )
