...
class Subscriber:
    ...
    async def send(self, chunk: AudioChunk):
        if self.queue.full():
...
class AudioBus:
    ...
    def set_ignore(self, subscriber_id: str, sources: set[str]):
    ...
    async def ingest_i16(self, source_id: str, pcm_i16: np.ndarray, sr: int):
    ...
    async def _loop(self, period_ms: int):
        try:
    ...
    def start(self, period_ms: int = 20):
