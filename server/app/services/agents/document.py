...
def _lint_latex_strings(src: str) -> None:
    ...
    if re.search(r"\\\\\\\\\\\\+", src):
        raise RuntimeError(
            "Suspicious 6+ backslashes found; likely double-escaped newline."
        )
...
"""
    ...
    enhanced_function = (
        imports_and_helper + doc_building_logic + compilation_boilerplate
    )
...
def _enhance_args_class(args_code: str, document_type: str) -> str:
    ...
    template_description = (
        f"A {document_type} template for creating professional documents."
    )
...
def create_template_tool(document_type: str) -> Any:
    ...
    async def generate(
        args_code: str = Field(
            description="Complete Python code for the Args class (pydantic BaseModel) with all required fields and descriptions"
        ),
        render_code: str = Field(
            description="Complete Python code for the render(args: Args) -> bytes function using PyLaTeX"
        ),
    ) -> str:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if not result.get("valid", False):
                            ...
                            detailed_error = (
                                f"Template validation failed: {error_message}"
                            )
                        ...
                        try:
                            ...
                            detailed_error = (
                                f"Template validation request failed: {error_message}"
                            )
                            ...
                            try:
                                ...
                                raise RuntimeError(
                                    f"Template validation request failed with status {response.status}. Response: {response_text}"
                                )
                            except:
                                # Final fallback if we can't parse the error response
                                raise RuntimeError(
                                    f"Template validation request failed with status {response.status}. Parse error: {parse_error}"
                                )
        ...
        logger.info(
            f"✓ Generated and enhanced template with {args_fields} Args fields and {render_lines} render lines"
        )
...
async def run_document_agent(
    ...
    system_prompt = (
        ...
        .replace("Context: Pitch deck template", f"Context: {document_type} template")
    )
    ...
    tools = [create_template_tool(document_type)]
    ...
    def tool_use_behavior(
        ...
        completed_required = document_progress.get(
            "args_code", False
        ) and document_progress.get("render_code", False)
...
"""
    ...
    logger.info(
        f"Document generation completed: Args={args_completed}, Render={render_completed}"
    )
