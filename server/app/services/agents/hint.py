...
async def run_hint_agent(
    ...
    try:
        ...
        user_persona: Personas | None = None
        ...
        if user_persona:
            context_parts.append(
                f"USER PERSONA: {user_persona.name} (the person practicing)"
            )
        ...
        if assistant_personas:
            ...
            context_parts.append(
                f"ASSISTANT PERSONAS: {', '.join(assistant_names)} (the people the user is practicing with)"
            )
            for assistant in assistant_personas:
                if assistant.description:
                    context_parts.append(
                        f"{assistant.name} DESCRIPTION: {assistant.description}"
                    )
        ...
        def tool_use_behavior(
            ...
            completed_required = all(
                hint_progress.get(tool, False) for tool in required_tools
            )
            logger.info(
                f"Tool use behavior check: required_tools={required_tools}, completed_required={completed_required}, hint_progress={hint_progress}"
            )
        ...
        with trace("Hint"):
            result = await Runner.run(
                hint_agent.agent(), input=full_conversation_history
            )

        logger.info(f"Successfully generated hints for message {message_id}")

        # Check if all required tools were called
        required_tools = ["dif_low", "dif_high"]
        completed_tools = [
            tool for tool in required_tools if hint_progress.get(tool, False)
        ]
        logger.info(
            f"Hint generation completed: {len(completed_tools)}/{len(required_tools)} required tools called"
        )
        ...
        if len(completed_tools) < len(required_tools):
            missing_tools = [
                tool for tool in required_tools if not hint_progress.get(tool, False)
            ]
        ...
        if dif_low_hints:
            low_hint = Hints(
                message_id=message_id, contents=dif_low_hints, difficulty="low"
            )
            session.add(low_hint)
            hint_ids.append(str(low_hint.id))
            saved_hints.append(
                {"id": str(low_hint.id), "difficulty": "low", "content": dif_low_hints}
            )
            logger.info(
                f"Created low difficulty hint with ID: {low_hint.id} and {len(dif_low_hints)} hint items"
            )

        if dif_high_hints:
            high_hint = Hints(
                message_id=message_id, contents=dif_high_hints, difficulty="high"
            )
            ...
            logger.info(
                f"Created high difficulty hint with ID: {high_hint.id} and {len(dif_high_hints)} hint items"
            )

        session.commit()

        logger.info(f"Successfully saved {len(saved_hints)} hint(s) to database")
