...
async def _upload_pdf_to_supabase_storage(pdf_bytes: bytes, doc_id: str) -> None:
    ...
    if not all([supabase_url, service_role_key]):
        ...
        logger.error(
            f"Missing: url={bool(supabase_url)}, service_key={bool(service_role_key)}"
        )
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        response = await client.post(storage_url, content=pdf_bytes, headers=headers)
        response.raise_for_status()

    logger.info(
        f"Successfully uploaded PDF {file_key} to Supabase Storage bucket {bucket_name}"
    )
...
def create_scenario_tool(scenario_id: uuid.UUID) -> Any:
    ...
    async def generate_scenario(
        ...
        problem_statement: str = Field(
            description="The detailed problem statement for the scenario"
        ),
    ) -> str:
...
def create_objectives_tool(scenario_id: uuid.UUID) -> Any:
    ...
    async def generate_objectives(
        objectives: list[str] = Field(
            description="List of learning objectives for the scenario"
        ),
    ) -> str:
        ...
        logger.info(
            f"✓ Generated {len(objectives)} objectives: {objectives[:2] if objectives else 'None'}..."
        )
...
def create_persona_prompt_tool(
    persona_id: uuid.UUID, persona_alias: str, persona_name: str
) -> Any:
    """Create a function tool for generating a prompt for a specific persona using an alias."""

    async def generate_persona_prompt(
        prompt: str = Field(
            description=f"Custom prompt for the {persona_alias} persona"
        ),
    ) -> str:
        ...
        logger.info(
            f"✓ Generated prompt for {persona_alias} ({persona_name}): {prompt[:50]}..."
        )
...
def _humanize(s: str) -> str:
    ...
    return " ".join(
        w.capitalize() for w in s.replace("_", " ").replace("-", " ").split()
    )
...
async def create_document_generation_tool(
    *,
    ...
    spec = (
        r.json()
    )  # has "fields", "json_schema", "default_filename", "template_description"
    ...
    tool_title = (
        default_filename or parameter_name or f"Template {str(template_id)[:8]}"
    )
    ...
    description = (
        template_desc or f"Generate document using {_humanize(tool_title)} template."
    )
    ...
    async def _impl(validated_args: BaseModel) -> str:
        ...
        try:
            ...
            document = Documents(content=text_content, title=doc_name, profile_id=None)
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Supabase Storage upload failed for document {document.id}: {storage_error}"
                    )
                ...
                logger.info(
                    f"Created document {document.id} for parameter {parameter_name}"
                )
            ...
            msg = (
                f"Documents service error: {e.response.status_code} - {e.response.text}"
            )
            logger.error(msg)
            return f"Error: {msg}"
        except Exception as e:
...
async def create_document_tool_for_scenario(
    scenario_id: uuid.UUID, session: Session
    ...
    try:
        ...
        if scenario_id in available_template_ids:
            ...
            scenario_name = (
                scenario.title if scenario else f"scenario_{str(scenario_id)[:8]}"
            )

            # Get template spec for metadata
            r = await HTTPX_CLIENT.get(
                f"{documents_service_url}/templates/{scenario_id}/spec"
            )
            r.raise_for_status()
            spec = r.json()
            ...
            tool_title = (
                default_filename or scenario_name or f"Scenario {str(scenario_id)[:8]}"
            )
            tool_name = f"{tool_title.lower().replace(' ', '_').replace('-', '_')}_doc"
            description = (
                template_desc
                or f"Generate document using {_humanize(tool_title)} template."
            )
            ...
            tools.append(tool)
            tool_metadata.append({"name": tool_name, "description": description})
            logger.info(
                f"Created document tool for scenario {scenario_name} ({scenario_id})"
            )
        else:
...
def calculate_persona_aliases(
    persona_ids: list[uuid.UUID], session: Session
) -> dict[uuid.UUID, str]:
...
async def create_scenario_tools(
    ...
    for persona_id in persona_ids:
        ...
        if persona_alias:
            ...
            if persona:
                persona_tool = create_persona_prompt_tool(
                    persona_id, persona_alias, persona.name
                )
                tools.append(persona_tool)
                logger.info(
                    f"Created persona prompt tool for {persona_alias} ({persona.name})"
                )
            else:
                logger.error(
                    f"Persona {persona_id} not found in database - this will cause scenario generation to fail"
                )
        else:
    ...
    if generate_documents:
        ...
        ) = await create_document_tool_for_scenario(scenario_id, session)
...
async def run_scenario_agent(
    ...
    try:
        ...
        parameter_history = get_parameter_history_from_field_values(
        ...
        try:
            scenario_tools, document_tool_metadata = await create_scenario_tools(
                scenario_id, persona_ids, fresh_session, generate_documents
            )
        finally:
            fresh_session.close()
        ...
        _emit_progress_fire_and_forget(
            "scenario_progress",
            {
                ...
                "document_tools_count": len(
                    document_tool_metadata
                ),  # Should be 1 for single document tool
            },
        )
        ...
        completed_required = [
            tool for tool in required_tools if scenario_progress.get(tool, False)
        ]
        logger.info(
            f"Scenario generation completed: {len(completed_required)}/{len(required_tools)} required tools called"
        )
        logger.info(f"Required tools: {required_tools}")
        logger.info(f"Completed tools: {completed_required}")
        logger.info(
            f"Document tools created: {[tool['name'] for tool in document_tool_metadata]}"
        )
        ...
        if len(completed_required) < len(required_tools):
            missing_tools = [
                ...
                if not scenario_progress.get(tool, False)
            ]
        ...
        for fv in field_values:
            ...
            if field and field.field_type == "document":
                ...
                logger.info(
                    f"Field {field_id} is not a document field (type: {field.field_type if field else 'not found'})"
                )
        ...
        return {
            "success": True,
            "message": "Successfully generated scenario",
